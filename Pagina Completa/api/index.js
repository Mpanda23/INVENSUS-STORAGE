...
app.delete('/sucursal3/borrar/:idsucursal', (req, res)=>{
  ...
  app.post('/usuarios/registro', async (req, res) => {
    const { usu_nombre, usu_correo, usu_contrasena } = req.body;
   
    try {
        const hashedPassword = await bcrypt.hash(usu_contrasena, saltRounds);

        const query = 'INSERT INTO usuarios (usu_nombre, usu_correo, usu_contrasena) VALUES (?, ?, ?)';
        conexion.query(query, [usu_nombre, usu_correo, hashedPassword], (error) => {
            if (error) {
                console.error(error.message);
...
app.put('/proplato/:id/productos', (req, res) => {
    ...
    SET p.cantidad = 
        CASE
            WHEN (p.idproducto = pp.idproducto AND p.cantidad - pp.cantidad < 0) THEN 0
            ELSE (p.cantidad - pp.cantidad)
        END
    ...
    SET p.cantidad = 
        CASE
            WHEN (p.idproducto = pp.idproducto2 AND p.cantidad - pp.cantidad2 < 0) THEN 0
            ELSE (p.cantidad - pp.cantidad2)
        END
    WHERE pp.proplaid = ${proplaid};
    `;
    ...
    SET p.cantidad = 
        CASE
            WHEN (p.idproducto = pp.idproducto3 AND p.cantidad - pp.cantidad3 < 0) THEN 0
            ELSE (p.cantidad - pp.cantidad3)
        END
    WHERE pp.proplaid = ${proplaid};
    `;
    ...
    SET p.cantidad = 
        CASE
            WHEN (p.idproducto = pp.idproducto4 AND p.cantidad - pp.cantidad4 < 0) THEN 0
            ELSE (p.cantidad - pp.cantidad4)
        END
    WHERE pp.proplaid = ${proplaid};
    `;
    ...
    SET p.cantidad = 
        CASE
            WHEN (p.idproducto = pp.idproducto5 AND p.cantidad - pp.cantidad5 < 0) THEN 0
            ELSE (p.cantidad - pp.cantidad5)
        END
    WHERE pp.proplaid = ${proplaid};
    `;
    ...
    SET p.cantidad = 
        CASE
            WHEN (p.idproducto = pp.idproducto6 AND p.cantidad - pp.cantidad6 < 0) THEN 0
            ELSE (p.cantidad - pp.cantidad6)
        END
    WHERE pp.proplaid = ${proplaid};
    `;
    ...
    SET p.cantidad = 
        CASE
            WHEN (p.idproducto = pp.idproducto7 AND p.cantidad - pp.cantidad7 < 0) THEN 0
            ELSE (p.cantidad - pp.cantidad7)
        END
    WHERE pp.proplaid = ${proplaid};
    `;
    ...
    SET p.cantidad = 
        CASE
            WHEN (p.idproducto = pp.idproducto8 AND p.cantidad - pp.cantidad8 < 0) THEN 0
            ELSE (p.cantidad - pp.cantidad8)
        END
    WHERE pp.proplaid = ${proplaid};
    `;
    ...
    SET p.cantidad = 
        CASE
            WHEN (p.idproducto = pp.idproducto9 AND p.cantidad - pp.cantidad9 < 0) THEN 0
            ELSE (p.cantidad - pp.cantidad9)
        END
    WHERE pp.proplaid = ${proplaid};
    `;
